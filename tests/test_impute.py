...
explanatory_rasters = [
    os.path.join(DATA, x)
    for x in "dtmean8_12.img gt_dem.img int_cnl_euc.img pmean_all.img "
    "p_n.img tmax8.img tmean_all.img".split()
]
response_raster = os.path.join(DATA, "responses.tif")
TMPOUT = "/tmp/pyimpute_test"

# Integration tests demonstrating a simplified example
...
def test_load_training_vector():
    ...
    train_xs, train_y = load_training_vector(
        response_shapes, explanatory_rasters, response_field="ZONE"
    )
...
def test_stratified_sample_raster():
    ...
    selected = stratified_sample_raster(
        response_raster, target_sample_size=20, min_sample_proportion=0.1
    )
