...
explanatory_rasters = [os.path.join(DATA, x) for x in 
    "dtmean8_12.img gt_dem.img int_cnl_euc.img pmean_all.img p_n.img tmax8.img tmean_all.img".split()]
...
def test_load_training_vector():
    ...
    train_xs, train_y = load_training_vector(response_shapes, 
        explanatory_rasters, response_field='ZONE')

    assert train_xs.shape == (245, 7)
    assert train_y.shape == (245, )
...
def test_stratified_sample_raster():
    ...
    selected = stratified_sample_raster(response_raster,
        target_sample_size=20, min_sample_proportion=0.1)
